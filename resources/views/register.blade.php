				...
				<div class="card">
					<div class="card-block">
						<auth-form
								...
								:data="{}"
								inline-template>
							<form class="form-horizontal" role="form" method="POST" action="http://localhost:8000/register" novalidate>
                                {{csrf_field()}}
								...
								<div class="auth-body">
                                    <div class="form-group" :class="{'has-danger': errors.has('text'), 'has-success': fields.text && fields.text.valid }">
										<p align="center" style="color:#4F4F4F;"><b id="textToType" style="font-family: Jura;font-style: normal;font-weight: normal;font-size: 16px;line-height: 22px;">The way i type can authenticate me on this website.</b></p>
										<div class="input-group input-group--custom" style="border-radius: 4px;">
                                            <input type="text" v-model="form.text" v-validate="'required'" class="form-control" :class="{'form-control-danger': errors.has('text'), 'form-control-success': fields.text && fields.text.valid}" id="text" name="text" placeholder="Type the text loaded above" style="border-left: 1px solid #b9c8de; border-radius: 4px;" oninput="highlight()">
										</div>
									...
									<div class="form-group">
										...
										<button type="submit" class="btn btn-primary btn-block btn-spinner" id="btn-register" disabled><i class="fa" onsubmit="check()"></i>Next</button>
                                        <br>
                                    </div>
								</div>
							</form>
...
<script type="text/javascript">
    ...
    var highlight = function(e){
        ...
        textToType.innerHTML = "<span class='highlight'>" + text.substring(0, length)+"</span>" + text.substring(length);
