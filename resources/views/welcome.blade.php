...
<head>
	...
	<link rel="stylesheet" href="{{ URL::asset('css/typedna.css') }}" type="text/css">
    <script type="text/javascript" src="{{ URL::asset('js/typedna.js') }}"></script>
		...
		function enrollNext() {
			...
			if (usernameEnroll && usernameEnroll.value.length > 5) {
				if (document.getElementById('iagree').checked === true) {
					document.location = 'enroll?user=' + usernameEnroll.value;
				} else {
			...
			if (usernameVerify && usernameVerify.value.length > 5) {
				...
				ajaxCall(params, function(result) {
					if(result && result.result > 0) {
						document.location = 'verify?user=' + usernameVerify.value;
					}else{
